module github.com/hbstack/header/modules/search

go 1.18

require (
	github.com/gohugoio/hugo-mod-bootstrap-scss/v5 v5.20300.20000 // indirect
	github.com/hbstack/header v0.1.1 // indirect
	github.com/hbstack/search v0.1.1 // indirect
	github.com/hugomods/base v0.1.0 // indirect
	github.com/hugomods/hugopress v0.1.0 // indirect
	github.com/hugomods/icons v0.6.0 // indirect
	github.com/hugomods/search v0.1.0 // indirect
	github.com/krisk/Fuse v6.6.2+incompatible // indirect
	github.com/twbs/icons v1.10.3 // indirect
)
